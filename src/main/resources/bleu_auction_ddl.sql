...
CREATE TABLE `ba_item` (
	...
	`wild_farm_status`	VARCHAR(1)	NULL	COMMENT 'W:자연산, F:양식',
	`item_status`	VARCHAR(1)	NULL	DEFAULT 'Y'	COMMENT 'Y,  N'
);


CREATE TABLE `ba_origin_country` (
	`origin_country_no`	BIGINT	NOT NULL	AUTO_INCREMENT PRIMARY KEY,
	`origin_country_name`	VARCHAR(20)	NOT NULL	COMMENT '국내, 국외'
);
...
CREATE TABLE `ba_origin_place` (
	`origin_place_no`	BIGINT	NOT NULL		AUTO_INCREMENT PRIMARY KEY,
	`origin_country_no`	BIGINT	NOT NULL	,
	`origin_place_name`	VARCHAR(20)	NOT NULL	COMMENT '국내 : 동해, 서해, 남해, 제주, 완도 /  국외 : 일본, 중국, 러시아, 노르웨이'
);
...
ALTER TABLE `ba_item` ADD CONSTRAINT `FK_ba_origin_country_TO_ba_item_1` FOREIGN KEY (
	`origin_country_no`
)
REFERENCES `ba_origin_country` (
	`origin_country_no`
);
...
ALTER TABLE `ba_origin_place` ADD CONSTRAINT `FK_ba_origin_country_TO_ba_origin_place_1` FOREIGN KEY (
	`origin_country_no`
)
REFERENCES `ba_origin_country` (
	`origin_country_no`
