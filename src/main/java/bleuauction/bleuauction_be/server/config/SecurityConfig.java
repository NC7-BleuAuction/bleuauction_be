...
public class SecurityConfig {
    ...
    public SecurityFilterChain filterChain(HttpSecurity http) throws Exception {
        http.authorizeHttpRequests(
                ...
                .addFilterBefore(
                        new APILoginFilter("/generateToken", new APILoginSuccessHandler(jwtUtils)),
                        UsernamePasswordAuthenticationFilter.class)
                .addFilterBefore(
                        new TokenCheckFilter(apiUserDetailsService, jwtUtils),
                        UsernamePasswordAuthenticationFilter.class)
                .addFilterBefore(
                        new RefreshTokenFilter("refreshToken", jwtUtils), TokenCheckFilter.class)
                // H2 사용을 하기 위한 옵션
                ...
                .cors(
                        httpSecurityCorsConfigurer ->
                                httpSecurityCorsConfigurer.configurationSource(
                                        corsConfigurationSource)); // CORS Spring Boot 설정
        return http.build();
    }
