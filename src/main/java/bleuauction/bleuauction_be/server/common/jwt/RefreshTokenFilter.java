...
public class RefreshTokenFilter extends OncePerRequestFilter {
  ...
  protected void doFilterInternal(HttpServletRequest request, HttpServletResponse response, FilterChain filterChain) throws ServletException, IOException {
    ...
    String renewRefreshToken = jwtUtils.isReissuanceRefreshToken(refreshToken) // 리프레쉬 토큰의 남은 유효기간이 재발급 기준에 해당할 경우
            ? jwtUtils.createRefreshToken(TokenMember.of(renewAccessToken), renewAccessToken)
            : refreshToken;

    sendToken(renewAccessToken, renewRefreshToken, response);
  }

  private void sendToken(String accessToeknValue, String refreshTokenValue, HttpServletResponse response) throws IOException{
