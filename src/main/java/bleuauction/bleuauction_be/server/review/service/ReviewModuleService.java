...
public class ReviewModuleService {
    ...
    public List<Review> selectReviewList(Long storeNo, int startPage) {
        ...
        List<Review> exitingReviewList =
                reviewRepository.findAllReviewsWithMembersByReviewStatus(
                        storeNo, ReviewStatus.Y, pageable);
        for (Review review : exitingReviewList) {
            Optional<List<Attach>> optionalAttachList =
                    Optional.ofNullable(
                            attachRepository.findAllByReviewAndFileStatus(review, FileStatus.Y));
            ...
            for (MultipartFile multipartFile : multipartFiles) {
                if (multipartFile.getSize() > 0) {
                    attachComponentService.saveWithReview(
                            insertReview, FileUploadUsage.REVIEW, multipartFile);
                }
            }
    ...
    public Review deleteReview(Long reviewNo) throws Exception {
        Review exitingReview =
                reviewRepository
                        .findByReviewNoAndReviewStatus(reviewNo, ReviewStatus.Y)
        ...
        Optional<List<Attach>> optionalAttachList =
                Optional.ofNullable(
                        attachRepository.findAllByReviewAndFileStatus(exitingReview, FileStatus.Y));
        if (optionalAttachList.isPresent()) {
        ...
        List<Answer> answerList =
                answerRepository.findAllByReviewAndStatus(exitingReview, AnswerStatus.Y);
